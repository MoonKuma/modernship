...
class NewUserLtvByDate(object):
    ...
    def __select_sql_data(self, table_name, *query_type):  # such functions are no longer exposed to outsides
        ...
        if self.channel != -1:
            where_clause += ' and channel=\'' + self.channel + '\' '
    ...
    def execute(self, file_name):
        ...
        try:
            self.__write_ltv_sheet('user_reten_pay_openid', 'OpenID新增按日LTV')
            self.__write_ltv_sheet('user_reten_pay', 'UID新增按日LTV(备查)')
            self.__write_ltv_sheet('user_reten_pay_openid', 'OpenID新增按日留存', 'reten')
            self.__write_ltv_sheet('user_reten_pay', 'UID新增按日留存(备查)', 'reten')
...
if __name__ == '__main__':
    ...
    channel_id = '1100'
    save_name = ConfParameters.ConfParameters().save_path + 'NewUserLtvRetenByDate_' + start + '_' + end + '_' + channel_id + '.xls'
