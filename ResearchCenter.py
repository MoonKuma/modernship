...
#####write xlwt
    ...
    def runwrite(self):
        ...
        self.sheetadd(unicode('非当天科技抽取（耗钻）')+self.zoneid[:10])
        self.xls_writetwo_dict(self.sheet, unicode('非当天抽取人数'), self.viplist, self.datelist, self.techpeople)
        self.xls_writetwo_dict(self.sheet, unicode('非当天老玩家人均每日抽取次数'), self.viplist, self.datelist, self.techavgtimes, 25)
        self.xls_writetwo_dict(self.sheet, unicode('非当天抽取人数占比'), self.viplist, self.datelist, self.techratio, 50)

        self.Tech_Gacha("2018-11-01","=")
        self.sheetadd(unicode('当天科技抽取（耗钻）')+self.zoneid[:10])
        self.xls_writetwo_dict(self.sheet, unicode('当天抽取人数'), self.viplist, self.datelist, self.techpeople)
        self.xls_writetwo_dict(self.sheet, unicode('当天老玩家人均每日抽取次数'), self.viplist, self.datelist, self.techavgtimes, 25)
        self.xls_writetwo_dict(self.sheet, unicode('当天抽取人数占比'), self.viplist, self.datelist, self.techratio, 50)
...
# | 2018-09-25 |    5 |       309 |       144 |   476 |
    ...
    def Tech_Gacha(self,begindate,symbol):

        self.datelist = []
        self.techratio = dict()
        self.techpeople = dict()
        self.techavgtimes = dict()
        sqlstr = 'select a.*,b.peoplenum,b.times,peoplenum/total,b.times/b.peoplenum from (select date,vip_level,count(distinct(uid)) as total from user_active_extend where date>=\"'+begindate+'\" and level>=6 and datediff(date,regdate)'+symbol+'15 '+self.channel+'and uid not in (select uid from modernship_stat_busi.uid_block_list) group by date,vip_level) as a inner join (select date,viplevel,sum(count) as peoplenum,sum(number*times) as times from ResearchCenter where pay>0 and datediff(date,regdate)'+symbol+'15 '+self.channel+'group by date,viplevel)as b on a.vip_level=b.viplevel and a.date=b.date;'
